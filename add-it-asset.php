...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($category) || empty($product) || empty($status)) {
        ...
        $stmt = mysqli_prepare($con, "INSERT INTO it_assets (branch_name, category, product, serial_id, label_name, status) VALUES (?, ?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "ssssss", $branch, $category, $product, $serial_id, $label, $status);

        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_close($stmt);
            header("Location: manage-it-assets.php?msg=added");
            exit();
        } else {
...
<head>
    ...
    <title>Add IT Asset | MPSC Bank Portal</title>
                    ...
                    <div class="it-assets-page">
                        <div class="it-assets-panel">
                            ...
                            <div class="ita-header">
                                <h1 class="ita-title"><i class="fas fa-plus-circle"></i> ADD PRODUCT</h1>
                            ...
                            <div class="ita-form-card">
                                <div class="ita-form-card-header">
                                    <i class="fas fa-clipboard-list"></i> New Asset Details
                                </div>
                                <div class="ita-form-card-body">
                                    <form method="POST" action="add-it-asset.php" id="addAssetForm">
                                        ...
                                        <div class="ita-form-grid">
                                                ...
                                                <select id="product" name="product" class="ita-input" required>
                                                    ...
                                                    <?php foreach ($products as $prod) { ?>
                                                        <option value="<?php echo htmlspecialchars($prod); ?>"
                                                            <?php echo (isset($_POST['product']) && $_POST['product'] === $prod) ? 'selected' : ''; ?>>
                                                ...
                                                <select id="status" name="status" class="ita-input" required>
                                                    ...
                                                    <?php foreach ($statuses as $st) { ?>
                                                        <option value="<?php echo htmlspecialchars($st); ?>"
                                                            <?php echo (isset($_POST['status']) && $_POST['status'] === $st) ? 'selected' : ''; ?>>
                                            ...
                                            <div class="ita-form-group">
                                                ...
                                                <input type="text" id="serial_id" name="serial_id"
                                                    value="<?php echo htmlspecialchars($_POST['serial_id'] ?? ''); ?>"
                                            ...
                                            <div class="ita-form-group">
                                                ...
                                                <input type="text" id="label_name" name="label_name"
                                                    value="<?php echo htmlspecialchars($_POST['label_name'] ?? ''); ?>"
                                                    class="ita-input" placeholder="Enter label name" />
                                            </div>
                                        </div>

                                        <div class="ita-form-actions">
                                            <button type="submit" class="ita-btn ita-btn-primary" id="btnSubmitAsset">
